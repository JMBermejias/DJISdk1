...
The aircraft sample includes a safety-gated control center with aircraft and remote-controller telemetry, battery/GPS/home-point checks, takeoff, landing, return-to-home, virtual-stick enablement, live camera preview, photo/video controls, and bounded timed capture automation. Commands that can move the aircraft require a registered SDK, connected aircraft and remote controller, valid home point, and the configured battery/GPS thresholds. Timed capture is bounded and can be stopped at any time.

The project is an Android application. Release artifacts are APK, AAB, and the UXSDK AAR. A Debian package is not produced because DJI MSDK V5 is an Android SDK, and `ABB` is not an Android artifact format.
